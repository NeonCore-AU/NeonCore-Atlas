...
pub trait ActiveKeyExchange: Send + Sync {
    ...
    /// Computes a shared secret without consuming the key exchange for REALITY-style ClientHello authentication.
    fn reality_shared_secret(&self, _peer_pub_key: &[u8]) -> Option<Result<SharedSecret, Error>> {
        None
    }
