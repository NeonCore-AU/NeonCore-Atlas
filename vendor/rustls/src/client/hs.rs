...
fn emit_client_hello_for_retry(
    ...
    if let Some(generator) = config.reality_session_id_generator.as_ref() {
        let Some(key_share) = key_share.as_ref() else {
            return Err(Error::General(
                "REALITY ClientHello requires a TLS 1.3 key share".into(),
            ));
        };
        if retryreq.is_some() || tls13_session.is_some() || ech_state.is_some() {
            return Err(Error::General(
                "REALITY ClientHello only supports the initial non-resumed TLS 1.3 hello".into(),
            ));
        }

        chp_payload.session_id = SessionId::from_compat_bytes([0u8; 32]);
        let zero_session_hello =
            HandshakeMessagePayload(HandshakePayload::ClientHello(chp_payload.clone()))
                .get_encoding();
        let session_id = generator.generate_session_id(
            key_share.as_ref(),
            &chp_payload.random.0,
            &zero_session_hello,
        )?;
        chp_payload.session_id = SessionId::from_compat_bytes(session_id);
    }
