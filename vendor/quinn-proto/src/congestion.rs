...
pub trait Controller: Send + Sync {
    ...
    /// Whether the controller provides its own pacing schedule.
    fn supports_custom_pacing(&self) -> bool {
        false
    }

    /// Return the next time an ack-eliciting datagram may be sent.
    #[allow(unused_variables)]
    fn pacing_delay(&mut self, now: Instant, bytes_to_send: u64, mtu: u16) -> Option<Instant> {
        None
    }

    /// Record that pacing-controlled bytes were emitted.
    #[allow(unused_variables)]
