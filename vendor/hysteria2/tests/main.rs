...
// Shared TLS configuration avoids rebuilding roots for every URL.
#[static_init::dynamic]
...
async fn http_get(mut sender: http1::SendRequest<String>, path: &str) -> Result<String> {
  ...
  // BodyExt::collect keeps response-body handling compact.
...
async fn test_url(url: String) -> Result<()> {
  ...
  // Parse configuration and connect.
  let config = Config::from_url(&url)?;
  let client = connect(&config).await?;

  // Establish a proxied TCP connection.
  ...
  // Send the request and fetch the observed IP.
